...
import ro.pub.elth.itee.dan.repository.NotaRepository;
...
/**
 * REST controller for managing {@link ro.pub.elth.itee.dan.domain.Nota}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class NotaResource {
     ...
     *         body the new nota, or with status {@code 400 (Bad Request)} if the
     *         nota has already an ID.
    ...
    public ResponseEntity<Nota> createNota(@Valid @RequestBody Nota nota) throws URISyntaxException {
        ...
        return ResponseEntity
                .created(new URI("/api/notas/" + result.getId())).headers(HeaderUtil
     ...
     *         the updated nota, or with status {@code 400 (Bad Request)} if the
     *         nota is not valid, or with status {@code 500 (Internal Server Error)}
     *         if the nota couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/notas")
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.ADMIN + "\")")
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the nota, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/notas/{id}")
    @PreAuthorize("hasRole(\"" + AuthoritiesConstants.ADMIN + "\")")
