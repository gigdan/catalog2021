...
import ro.pub.elth.itee.dan.repository.DisciplinaRepository;
import ro.pub.elth.itee.dan.web.rest.errors.BadRequestAlertException;
...
/**
 * REST controller for managing {@link ro.pub.elth.itee.dan.domain.Disciplina}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class DisciplinaResource {
     ...
     *         body the new disciplina, or with status {@code 400 (Bad Request)} if
     *         the disciplina has already an ID.
    ...
    public ResponseEntity<Disciplina> createDisciplina(@Valid @RequestBody Disciplina disciplina)
        ...
        return ResponseEntity
                .created(new URI("/api/disciplinas/" + result.getId())).headers(HeaderUtil
     ...
     *         the updated disciplina, or with status {@code 400 (Bad Request)} if
     *         the disciplina is not valid, or with status
     *         {@code 500 (Internal Server Error)} if the disciplina couldn't be
     *         updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body
     *         the disciplina, or with status {@code 404 (Not Found)}.
     */
